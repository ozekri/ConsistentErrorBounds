...
for n in tqdm(range(frames//2 -10)):
    ...
    #plt.legend()
    plt.tight_layout()
    plt.savefig(f"gif_saves2/{n}.png")
    plt.close()
    c0+=1
...
images = [Image.open(f"gif_saves2/{n}.png") for n in range(frames)]
images[0].save('gif_saves2/render.gif', save_all=True, append_images=images[1:], duration=100, loop=0)